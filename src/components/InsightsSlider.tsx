...
type InsightsSliderProps = {
  posts: PostMeta[]
}

export function InsightsSlider({ posts }: InsightsSliderProps) {
  return (
      ...
      <div className='w-full overflow-x-auto no-scrollbar scroll-smooth snap-x snap-mandatory pb-12'>
        <div className='flex px-6 md:px-12 lg:px-16 gap-6 w-max'>
          {posts.map((item) => (
            ...
            >
              <div className='w-full aspect-4/3 rounded-2xl overflow-hidden relative shadow-sm transition-transform duration-500 group-hover:scale-[1.02] bg-zinc-200'>
                <img
                  src={item.image}
                  alt={item.title}
